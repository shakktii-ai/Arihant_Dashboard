...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1️⃣ Always Update Onboarding Credits (Master Balance)
    const updatedOnboarding = await CompanyOnboarding.findOneAndUpdate(
      { companyId },
      { $inc: { creditsRemaining: amount } },
      { new: true, upsert: true }
    );

    // 2️⃣ Always Create/Update a Payment Record for Tracking
    // We check if a record exists to maintain consistency, but we always increment.
    await CompanyPayment.findOneAndUpdate(
      { companyId },
      {
        $inc: { creditsRemaining: amount },
        $set: {
          paymentProof: `MANUAL_ADDITION_BY_SUPERADMIN_${new Date().getTime()}`,
    ...
    const updatedCredits = updatedOnboarding.creditsRemaining;
    ...
    await Invoice.create({
      ...
      items: [
        {
          description: `Credit Purchase (${amount} credits) (Manual Live Addition)`,
          quantity: amount,
