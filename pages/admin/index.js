  ...
  async function loadCompanyInfo() {
    try {
      ...
      if (data.ok && data.onboarding) {
        setCompanyInfo(data.onboarding);
        setCredits(data.onboarding.creditsRemaining || 0);
      }
    } catch (err) {
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <div className="flex flex-col gap-2">
              <img src="/arihant-logo.png" alt="Arihant Superstructures" className="h-12 md:h-16 w-fit" />
              <p className="text-[11px] md:text-[12px] text-slate-500 font-medium tracking-[0.2em] uppercase opacity-70 flex items-center gap-3">
          ...
          <div className="flex items-center gap-4">
            <div className="bg-indigo-50 border border-indigo-100 rounded-lg px-4 py-2 flex items-center gap-2">
              ...
              <span className="text-xl font-bold text-indigo-600">{credits}</span>
            </div>

            <div className="flex gap-2">
              {credits === 0 && (
                <button
                  onClick={() => router.push("/admin/company/onboarding?step=4")}
                  className="px-4 py-2 bg-amber-500 text-white rounded-md shadow hover:bg-amber-600 transition-all font-medium flex items-center gap-2"
                >
              ...
              <button
                onClick={() => {
                  if (credits <= 0) {
                    alert("You have 0 credits. Please recharge to create more interviews.");
                    return;
                  }
                  setShowCreate(true);
                }}
                disabled={credits <= 0}
                className={`px-4 py-2 text-white rounded-md shadow transition-all ${credits <= 0
                      ...
                      <tbody>
                        ...
                        {pageItems.map(r => (
                          <tr key={r._id} className="border-t">
                            ...
                            <td className="py-3 pr-4">
                              <div className="flex gap-2">
                                <button onClick={() => openReportModal(r)} className="px-3 py-1 bg-indigo-600 text-white rounded text-sm hover:bg-indigo-700">View</button>
                                <button
