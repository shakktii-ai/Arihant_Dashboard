...
export default function AdminIndex() {
  ...
  const handleBuyCredits = async () => {
    ...
    try {
      ...
      // 1. Create Order
      const orderRes = await fetch("/api/admin/company/razorpay-order", {
        method: "POST",
        credentials: "include",
      });
      ...
      const options = {
        ...
        handler: async function (response) {
          // 3. Verify Payment
          try {
            const verifyRes = await fetch("/api/admin/company/razorpay-verify", {
              method: "POST",
            ...
            if (verifyData.ok) {
              alert("Payment successful! Credits added & Invoice generated.");
              // Refresh company info & credits
